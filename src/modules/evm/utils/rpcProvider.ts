...
class RPCManager {
  private providerCache: Map<string, ethers.JsonRpcProvider> = new Map();
  private urlStats: Map<string, UrlStats> = new Map();
  private urlOrder: Map<number | string, string[]> = new Map();
  private roundRobinIndex: Map<number | string, number> = new Map();
  private chainDeadUntil: Map<number | string, number> = new Map();
  private probeInFlight: Set<string> = new Set();
  ...
  private getStableOrder(chainId: number | string, urls: string[]): string[] {
    const unique = Array.from(new Set(urls.filter(Boolean)));
    const existing = this.urlOrder.get(chainId);
    if (existing && existing.length === unique.length && unique.every(u => existing.includes(u))) {
      return existing;
    }
    this.urlOrder.set(chainId, unique);
    this.roundRobinIndex.set(chainId, 0);
    return unique;
  }

  private pickUrls(chainId: number | string, urls: string[]): string[] {
    const stableOrder = this.getStableOrder(chainId, urls);
    const available = stableOrder.filter(u => this.isUrlAvailable(u));

    if (available.length === 0) return [];

    const idx = (this.roundRobinIndex.get(chainId) ?? 0) % available.length;
    this.roundRobinIndex.set(chainId, idx + 1);

    const primary = available[idx];
    const rest = available.filter(u => u !== primary);
