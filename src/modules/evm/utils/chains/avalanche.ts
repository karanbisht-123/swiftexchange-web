import { type ChainConfig } from '../Chainregistry';
import {
    getChainLogoUrlBySlug,
    getAssetLogoUrl
} from '../ChainUrlHelpers';

const slug = 'avalanchec';

const avalanche: ChainConfig = {
    chainId: 43114,
    name: 'Avalanche C-Chain',
    networkType: 'mainnet',
    available: true,
    swapEnabled: true,
    slug: 'avalanche',
    skipChainName: 'avalanche',
    rpcUrl: 'https://api.avax.network/ext/bc/C/rpc',
    fallbackRpcUrls: [
        'https://avalanche-c-chain.publicnode.com',
        'https://1rpc.io/avax/c',
        'https://avalanche.drpc.org',
    ],
    blockExplorerUrl: 'https://snowscan.xyz',
    nativeCurrency: {
        name: 'Avalanche',
        symbol: 'AVAX',
        decimals: 18,
        logoURI: getAssetLogoUrl(slug, '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7'),
        wrappedAddress: '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7',
        coingeckoId: 'avalanche-2',
    },
    logoURI: getChainLogoUrlBySlug(slug),
    coingeckoPlatform: 'avalanche',
    website: 'https://www.avax.network/',
    description: 'Avalanche is an open-source platform for launching decentralized applications and enterprise blockchain deployments in one interoperable, highly scalable ecosystem.',
    status: 'active',
    tags: ['dapp', 'staking-native'],
    links: [
        { name: 'github', url: 'https://github.com/ava-labs' },
        { name: 'x', url: 'https://x.com/avax' }
    ],
    tokens: {
        WAVAX: '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7',
        USDC: '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E',
        USDT_E: '0x9702230A24153011882159D7373727271D1373b5',
    },
    assets: [
        {
            asset: 'c10009000_t0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7',
            type: 'ERC20',
            address: '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7',
            name: 'Wrapped AVAX',
            symbol: 'WAVAX',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7'),
            coingeckoId: 'wrapped-avax',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x8b82A291F83ca07Af22120ABa21632088fC92931',
            type: 'ERC20',
            address: '0x8b82A291F83ca07Af22120ABa21632088fC92931',
            name: 'Ether (Portal)',
            symbol: 'ETH',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x8b82A291F83ca07Af22120ABa21632088fC92931'),
            coingeckoId: 'ethereum',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x12AF5C1a232675f62F405b5812A80e7a6F75D746',
            type: 'ERC20',
            address: '0x12AF5C1a232675f62F405b5812A80e7a6F75D746',
            name: 'ROSE (Portal)',
            symbol: 'ROSE',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x12AF5C1a232675f62F405b5812A80e7a6F75D746'),
            coingeckoId: 'oasis-network',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x98a4d09036Cc5337810096b1D004109686E56Afc',
            type: 'ERC20',
            address: '0x98a4d09036Cc5337810096b1D004109686E56Afc',
            name: 'PancakeSwap Token (Portal)',
            symbol: 'CAKE',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x98a4d09036Cc5337810096b1D004109686E56Afc'),
            coingeckoId: 'pancakeswap-token',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x442F7f22b1EE2c842bEAFf52880d4573E9201158',
            type: 'ERC20',
            address: '0x442F7f22b1EE2c842bEAFf52880d4573E9201158',
            name: 'Binance Coin (Portal)',
            symbol: 'BNB',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x442F7f22b1EE2c842bEAFf52880d4573E9201158'),
            coingeckoId: 'binancecoin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x0950Fc1AD509358dAeaD5eB8020a3c7d8b43b9DA',
            type: 'ERC20',
            address: '0x0950Fc1AD509358dAeaD5eB8020a3c7d8b43b9DA',
            name: 'USD Coin (Portal from Solana)',
            symbol: 'USDCso',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0x0950Fc1AD509358dAeaD5eB8020a3c7d8b43b9DA'),
            coingeckoId: 'usd-coin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x6145E8a910aE937913426BF32De2b26039728ACF',
            type: 'ERC20',
            address: '0x6145E8a910aE937913426BF32De2b26039728ACF',
            name: 'USD Coin (Portal from BSC)',
            symbol: 'USDCbs',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x6145E8a910aE937913426BF32De2b26039728ACF'),
            coingeckoId: 'usd-coin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x60781C2586D68229fde47564546784ab3fACA982',
            type: 'ERC20',
            address: '0x60781C2586D68229fde47564546784ab3fACA982',
            name: 'Pangolin',
            symbol: 'PNG',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x60781C2586D68229fde47564546784ab3fACA982'),
            coingeckoId: 'pangolin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x70928E5B188def72817b7775F0BF6325968e563B',
            type: 'ERC20',
            address: '0x70928E5B188def72817b7775F0BF6325968e563B',
            name: 'LUNA (Portal)',
            symbol: 'LUNA',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0x70928E5B188def72817b7775F0BF6325968e563B'),
            coingeckoId: 'terra-luna',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x543672E9CBEC728CBBa9C3Ccd99ed80aC3607FA8',
            type: 'ERC20',
            address: '0x543672E9CBEC728CBBa9C3Ccd99ed80aC3607FA8',
            name: 'USD Coin (PoS) (Portal from Polygon)',
            symbol: 'USDCpo',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0x543672E9CBEC728CBBa9C3Ccd99ed80aC3607FA8'),
            coingeckoId: 'usd-coin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xA67BCC0D06d7d13A13A2AE30bF30f1B434f5a28B',
            type: 'ERC20',
            address: '0xA67BCC0D06d7d13A13A2AE30bF30f1B434f5a28B',
            name: 'Tether USD (Portal from BSC)',
            symbol: 'USDTbs',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xA67BCC0D06d7d13A13A2AE30bF30f1B434f5a28B'),
            coingeckoId: 'tether',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xA41a6c7E25DdD361343e8Cb8cFa579bbE5eEdb7a',
            type: 'ERC20',
            address: '0xA41a6c7E25DdD361343e8Cb8cFa579bbE5eEdb7a',
            name: 'Binance USD (Portal from BSC)',
            symbol: 'BUSDbs',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xA41a6c7E25DdD361343e8Cb8cFa579bbE5eEdb7a'),
            coingeckoId: 'binance-usd',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xB0a6e056B587D0a85640b39b1cB44086F7a26A1E',
            type: 'ERC20',
            address: '0xB0a6e056B587D0a85640b39b1cB44086F7a26A1E',
            name: 'Oddz',
            symbol: 'ODDZ',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xB0a6e056B587D0a85640b39b1cB44086F7a26A1E'),
            coingeckoId: 'oddz',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xB24CA28D4e2742907115fECda335b40dbda07a4C',
            type: 'ERC20',
            address: '0xB24CA28D4e2742907115fECda335b40dbda07a4C',
            name: 'USD Coin (Portal from Ethereum)',
            symbol: 'USDCet',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0xB24CA28D4e2742907115fECda335b40dbda07a4C'),
            coingeckoId: 'usd-coin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xb599c3590F42f8F995ECfa0f85D2980B76862fc1',
            type: 'ERC20',
            address: '0xb599c3590F42f8F995ECfa0f85D2980B76862fc1',
            name: 'UST (Portal)',
            symbol: 'UST',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0xb599c3590F42f8F995ECfa0f85D2980B76862fc1'),
            coingeckoId: 'terrausd',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xC17c30e98541188614dF99239cABD40280810cA3',
            type: 'ERC20',
            address: '0xC17c30e98541188614dF99239cABD40280810cA3',
            name: 'EverRise',
            symbol: 'RISE',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xC17c30e98541188614dF99239cABD40280810cA3'),
            coingeckoId: 'everrise',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xd039C9079ca7F2a87D632A9C0d7cEa0137bAcFB5',
            type: 'ERC20',
            address: '0xd039C9079ca7F2a87D632A9C0d7cEa0137bAcFB5',
            name: 'Ape-X',
            symbol: 'APE-X',
            decimals: 9,
            logoURI: getAssetLogoUrl(slug, '0xd039C9079ca7F2a87D632A9C0d7cEa0137bAcFB5'),
            coingeckoId: 'ape-x',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xF0FF231e3F1A50F83136717f287ADAB862f89431',
            type: 'ERC20',
            address: '0xF0FF231e3F1A50F83136717f287ADAB862f89431',
            name: 'Tether USD (Portal from Solana)',
            symbol: 'USDTso',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0xF0FF231e3F1A50F83136717f287ADAB862f89431'),
            coingeckoId: 'tether',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xF2F7CE610a091B94d41D69f4fF1129434a82E2f0',
            type: 'ERC20',
            address: '0xF2F7CE610a091B94d41D69f4fF1129434a82E2f0',
            name: 'Galaxy Goggle',
            symbol: 'GG',
            decimals: 9,
            logoURI: getAssetLogoUrl(slug, '0xF2F7CE610a091B94d41D69f4fF1129434a82E2f0'),
            coingeckoId: 'galaxy-goggle',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xf2f13f0B7008ab2FA4A2418F4ccC3684E49D20Eb',
            type: 'ERC20',
            address: '0xf2f13f0B7008ab2FA4A2418F4ccC3684E49D20Eb',
            name: 'MATIC (Portal from Polygon)',
            symbol: 'MATICpo',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xf2f13f0B7008ab2FA4A2418F4ccC3684E49D20Eb'),
            coingeckoId: 'matic-network',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xFE6B19286885a4F7F55AdAD09C3Cd1f906D2478F',
            type: 'ERC20',
            address: '0xFE6B19286885a4F7F55AdAD09C3Cd1f906D2478F',
            name: 'SOL (Portal)',
            symbol: 'SOL',
            decimals: 9,
            logoURI: getAssetLogoUrl(slug, '0xFE6B19286885a4F7F55AdAD09C3Cd1f906D2478F'),
            coingeckoId: 'solana',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xc7198437980c041c805A1EDcbA50c1Ce5db95118',
            type: 'ERC20',
            address: '0xc7198437980c041c805A1EDcbA50c1Ce5db95118',
            name: 'Tether USD',
            symbol: 'USDT.e',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0xc7198437980c041c805A1EDcbA50c1Ce5db95118'),
            coingeckoId: 'tether',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7',
            type: 'ERC20',
            address: '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7',
            name: 'TetherToken',
            symbol: 'USDt',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7'),
            coingeckoId: 'tether',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E',
            type: 'ERC20',
            address: '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E',
            name: 'USD Coin',
            symbol: 'USDC',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E'),
            coingeckoId: 'usd-coin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664',
            type: 'ERC20',
            address: '0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664',
            name: 'USD Coin',
            symbol: 'USDC.e',
            decimals: 6,
            logoURI: getAssetLogoUrl(slug, '0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664'),
            coingeckoId: 'usd-coin-ethereum-bridged',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x19860CCB0A68fd4213aB9D8266F7bBf05A8dDe98',
            type: 'ERC20',
            address: '0x19860CCB0A68fd4213aB9D8266F7bBf05A8dDe98',
            name: 'Binance USD',
            symbol: 'BUSD.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x19860CCB0A68fd4213aB9D8266F7bBf05A8dDe98'),
            coingeckoId: 'binance-usd',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xd586E7F844cEa2F87f50152665BCbc2C279D8d70',
            type: 'ERC20',
            address: '0xd586E7F844cEa2F87f50152665BCbc2C279D8d70',
            name: 'Dai Stablecoin',
            symbol: 'DAI.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xd586E7F844cEa2F87f50152665BCbc2C279D8d70'),
            coingeckoId: 'dai',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x50b7545627a5162F82A992c33b87aDc75187B218',
            type: 'ERC20',
            address: '0x50b7545627a5162F82A992c33b87aDc75187B218',
            name: 'Wrapped BTC',
            symbol: 'WBTC.e',
            decimals: 8,
            logoURI: getAssetLogoUrl(slug, '0x50b7545627a5162F82A992c33b87aDc75187B218'),
            coingeckoId: 'wrapped-bitcoin',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x02D980A0D7AF3fb7Cf7Df8cB35d9eDBCF355f665',
            type: 'ERC20',
            address: '0x02D980A0D7AF3fb7Cf7Df8cB35d9eDBCF355f665',
            name: 'SHIBA INU',
            symbol: 'SHIB.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x02D980A0D7AF3fb7Cf7Df8cB35d9eDBCF355f665'),
            coingeckoId: 'shiba-inu',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x5947BB275c521040051D82396192181b413227A3',
            type: 'ERC20',
            address: '0x5947BB275c521040051D82396192181b413227A3',
            name: 'Chainlink Token',
            symbol: 'LINK.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x5947BB275c521040051D82396192181b413227A3'),
            coingeckoId: 'chainlink',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x8eBAf22B6F053dFFeaf46f4Dd9eFA95D89ba8580',
            type: 'ERC20',
            address: '0x8eBAf22B6F053dFFeaf46f4Dd9eFA95D89ba8580',
            name: 'Uniswap',
            symbol: 'UNI.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x8eBAf22B6F053dFFeaf46f4Dd9eFA95D89ba8580'),
            coingeckoId: 'uniswap',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xD24C2Ad096400B6FBcd2ad8B24E7acBc21A1da64',
            type: 'ERC20',
            address: '0xD24C2Ad096400B6FBcd2ad8B24E7acBc21A1da64',
            name: 'Frax',
            symbol: 'FRAX',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xD24C2Ad096400B6FBcd2ad8B24E7acBc21A1da64'),
            coingeckoId: 'frax',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x1C20E891Bab6b1727d14Da358FAe2984Ed9B59EB',
            type: 'ERC20',
            address: '0x1C20E891Bab6b1727d14Da358FAe2984Ed9B59EB',
            name: 'TrueUSD',
            symbol: 'TUSD',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x1C20E891Bab6b1727d14Da358FAe2984Ed9B59EB'),
            coingeckoId: 'true-usd',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x63a72806098Bd3D9520cC43356dD78afe5D386D9',
            type: 'ERC20',
            address: '0x63a72806098Bd3D9520cC43356dD78afe5D386D9',
            name: 'Aave Token',
            symbol: 'AAVE.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x63a72806098Bd3D9520cC43356dD78afe5D386D9'),
            coingeckoId: 'aave',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xcf799767d366d789e8B446981C2D578E241fa25c',
            type: 'ERC20',
            address: '0xcf799767d366d789e8B446981C2D578E241fa25c',
            name: 'Decentralized USD',
            symbol: 'USDD',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xcf799767d366d789e8B446981C2D578E241fa25c'),
            coingeckoId: 'usdd',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x88128fd4b259552A9A1D457f435a6527AAb72d42',
            type: 'ERC20',
            address: '0x88128fd4b259552A9A1D457f435a6527AAb72d42',
            name: 'Maker',
            symbol: 'MKR.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x88128fd4b259552A9A1D457f435a6527AAb72d42'),
            coingeckoId: 'maker',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x8a0cAc13c7da965a312f08ea4229c37869e85cB9',
            type: 'ERC20',
            address: '0x8a0cAc13c7da965a312f08ea4229c37869e85cB9',
            name: 'Graph Token',
            symbol: 'GRT.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x8a0cAc13c7da965a312f08ea4229c37869e85cB9'),
            coingeckoId: 'the-graph',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x98443B96EA4b0858FDF3219Cd13e98C7A4690588',
            type: 'ERC20',
            address: '0x98443B96EA4b0858FDF3219Cd13e98C7A4690588',
            name: 'Basic Attention Token',
            symbol: 'BAT.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x98443B96EA4b0858FDF3219Cd13e98C7A4690588'),
            coingeckoId: 'basic-attention-token',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xd501281565bf7789224523144Fe5D98e8B28f267',
            type: 'ERC20',
            address: '0xd501281565bf7789224523144Fe5D98e8B28f267',
            name: '1INCH Token',
            symbol: '1INCH.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xd501281565bf7789224523144Fe5D98e8B28f267'),
            coingeckoId: '1inch',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x249848BeCA43aC405b8102Ec90Dd5F22CA513c06',
            type: 'ERC20',
            address: '0x249848BeCA43aC405b8102Ec90Dd5F22CA513c06',
            name: 'Curve DAO Token',
            symbol: 'CRV.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x249848BeCA43aC405b8102Ec90Dd5F22CA513c06'),
            coingeckoId: 'curve-dao-token',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xc3048E19E76CB9a3Aa9d77D8C03c29Fc906e2437',
            type: 'ERC20',
            address: '0xc3048E19E76CB9a3Aa9d77D8C03c29Fc906e2437',
            name: 'Compound',
            symbol: 'COMP.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xc3048E19E76CB9a3Aa9d77D8C03c29Fc906e2437'),
            coingeckoId: 'compound-governance-token',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x596fA47043f99A4e0F122243B841E55375cdE0d2',
            type: 'ERC20',
            address: '0x596fA47043f99A4e0F122243B841E55375cdE0d2',
            name: 'ZRX',
            symbol: 'ZRX.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x596fA47043f99A4e0F122243B841E55375cdE0d2'),
            coingeckoId: '0x',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xBeC243C995409E6520D7C41E404da5dEba4b209B',
            type: 'ERC20',
            address: '0xBeC243C995409E6520D7C41E404da5dEba4b209B',
            name: 'Synthetix Network Token',
            symbol: 'SNX.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xBeC243C995409E6520D7C41E404da5dEba4b209B'),
            coingeckoId: 'havven',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x3Bd2B1c7ED8D396dbb98DED3aEbb41350a5b2339',
            type: 'ERC20',
            address: '0x3Bd2B1c7ED8D396dbb98DED3aEbb41350a5b2339',
            name: 'UMA Voting Token v1',
            symbol: 'UMA.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x3Bd2B1c7ED8D396dbb98DED3aEbb41350a5b2339'),
            coingeckoId: 'uma',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x9eAaC1B23d935365bD7b542Fe22cEEe2922f52dc',
            type: 'ERC20',
            address: '0x9eAaC1B23d935365bD7b542Fe22cEEe2922f52dc',
            name: 'yearn.finance',
            symbol: 'YFI.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x9eAaC1B23d935365bD7b542Fe22cEEe2922f52dc'),
            coingeckoId: 'yearn-finance',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x39fC9e94Caeacb435842FADeDeCB783589F50f5f',
            type: 'ERC20',
            address: '0x39fC9e94Caeacb435842FADeDeCB783589F50f5f',
            name: 'Kyber Network Crystal v2',
            symbol: 'KNC',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x39fC9e94Caeacb435842FADeDeCB783589F50f5f'),
            coingeckoId: 'kyber-network-crystal',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x37B608519F91f70F2EeB0e5Ed9AF4061722e4F76',
            type: 'ERC20',
            address: '0x37B608519F91f70F2EeB0e5Ed9AF4061722e4F76',
            name: 'SushiToken',
            symbol: 'SUSHI.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x37B608519F91f70F2EeB0e5Ed9AF4061722e4F76'),
            coingeckoId: 'sushi',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x340fE1D898ECCAad394e2ba0fC1F93d27c7b717A',
            type: 'ERC20',
            address: '0x340fE1D898ECCAad394e2ba0fC1F93d27c7b717A',
            name: 'Orbs',
            symbol: 'ORBS',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x340fE1D898ECCAad394e2ba0fC1F93d27c7b717A'),
            coingeckoId: 'orbs',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x5fC17416925789E0852FBFcd81c490ca4abc51F9',
            type: 'ERC20',
            address: '0x5fC17416925789E0852FBFcd81c490ca4abc51F9',
            name: 'inSure',
            symbol: 'SURE',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x5fC17416925789E0852FBFcd81c490ca4abc51F9'),
            coingeckoId: 'insure',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xCE1bFFBD5374Dac86a2893119683F4911a2F7814',
            type: 'ERC20',
            address: '0xCE1bFFBD5374Dac86a2893119683F4911a2F7814',
            name: 'Spell Token',
            symbol: 'SPELL',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xCE1bFFBD5374Dac86a2893119683F4911a2F7814'),
            coingeckoId: 'spell-token',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x214DB107654fF987AD859F34125307783fC8e387',
            type: 'ERC20',
            address: '0x214DB107654fF987AD859F34125307783fC8e387',
            name: 'Frax Share',
            symbol: 'FXS',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x214DB107654fF987AD859F34125307783fC8e387'),
            coingeckoId: 'frax-share',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xB44a9B6905aF7c801311e8F4E76932ee959c663C',
            type: 'ERC20',
            address: '0xB44a9B6905aF7c801311e8F4E76932ee959c663C',
            name: 'Anyswap',
            symbol: 'ANY',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xB44a9B6905aF7c801311e8F4E76932ee959c663C'),
            coingeckoId: 'anyswap',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x2147EFFF675e4A4eE1C2f918d181cDBd7a8E208f',
            type: 'ERC20',
            address: '0x2147EFFF675e4A4eE1C2f918d181cDBd7a8E208f',
            name: 'AlphaToken',
            symbol: 'ALPHA.e',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x2147EFFF675e4A4eE1C2f918d181cDBd7a8E208f'),
            coingeckoId: 'alpha-finance',
            pairs: [],
        },
        {
            asset: 'c10009000_t0xd6070ae98b8069de6B494332d1A1a81B6179D960',
            type: 'ERC20',
            address: '0xd6070ae98b8069de6B494332d1A1a81B6179D960',
            name: 'beefy.finance',
            symbol: 'BIFI',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0xd6070ae98b8069de6B494332d1A1a81B6179D960'),
            coingeckoId: 'beefy-finance',
            pairs: [],
        },
        {
            asset: 'c10009000_t0x62edc0692BD897D2295872a9FFCac5425011c661',
            type: 'ERC20',
            address: '0x62edc0692BD897D2295872a9FFCac5425011c661',
            name: 'GMX',
            symbol: 'GMX',
            decimals: 18,
            logoURI: getAssetLogoUrl(slug, '0x62edc0692BD897D2295872a9FFCac5425011c661'),
            coingeckoId: 'gmx',
            pairs: [],
        },
    ],
};

export default avalanche;
