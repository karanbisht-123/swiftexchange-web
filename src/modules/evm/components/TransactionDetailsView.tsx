import { Check, Copy, ExternalLink, ArrowDownLeft } from 'lucide-react';
        ...
        <div className="flex flex-col h-full bg-secondary rounded-2xl overflow-hidden border border-color shadow-sm">
            <div className="bg-tertiary/30 p-6 flex flex-col items-center justify-center border-b border-color">
                <div className="w-16 h-16 rounded-full bg-tertiary flex items-center justify-center mb-4 shadow-inner">
                    <img
                        src={network === 'BNB' ? "https://tokens.pancakeswap.finance/images/0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c.png" : "https://coin-images.coingecko.com/coins/images/279/large/ethereum.png"}
                        alt={network}
                        className="w-10 h-10 rounded-full"
                    />
                </div>
                <div className="text-3xl font-bold text-primary tracking-tight">
                    {parseFloat(transaction.formattedAmount).toFixed(6)} <span className="text-lg text-muted font-medium">{transaction.asset}</span>
                </div>
                <div className="mt-2 flex items-center gap-2">
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${transaction.formattedAmount.startsWith('-')
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <a
                                href={`${network === 'BNB' ? 'https://bscscan.com/block/' : 'https://etherscan.io/block/'}${parseInt(transaction.blockNum, 16)}`}
                                target="_blank"
                                rel="noopener noreferrer"
