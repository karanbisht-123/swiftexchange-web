      ...
      return [
        {
          ...
          status: currentStep === 'approving' ? 'active' : currentStep === 'signing' ? 'done' : 'waiting',
        },
        {
  ...
  return (
    <div className="bg-tertiary rounded-3xl p-6 border border-divider/50 w-full max-w-full flex flex-col items-center animate-in fade-in duration-300 relative overflow-hidden min-h-[480px] justify-between">
      ...
      <div className="w-full bg-secondary border border-color rounded-2xl p-5 mt-6 z-10 flex items-center justify-between shadow-sm relative group overflow-hidden">
        <div className="flex items-center gap-3.5 min-w-0">
          <div className="relative flex-shrink-0">
            <img
              src={sellAsset?.logoURI || `https://ui-avatars.com/api/?name=${sellAsset?.symbol || 'Token'}&background=random`}
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-xl font-black text-primary leading-tight truncate">
              {sellAmount} <span className="text-xs text-muted font-bold tracking-normal">{sellAsset?.symbol}</span>
            </span>
        ...
        <div className="text-right flex flex-col items-end min-w-0">
          <span className="text-base font-black text-brand leading-none">
            {calculatedBuyAmount} <span className="text-[10px] text-brand/80 font-bold">{buyAsset?.symbol}</span>
          </span>
            ...
            return (
              <div key={idx} className="flex gap-4 items-start relative z-10">
                ...
                <div className="flex-1 min-w-0">
                  <h4
                    className={`text-xs font-black uppercase tracking-wider leading-none transition-colors ${isActive ? 'text-brand' : isCompleted ? 'text-primary' : 'text-muted'
                      }`}
                  >
                    {step.title}
                  </h4>
      ...
      <div className="w-full z-10 pt-4 border-t border-white/5 space-y-4">
        ...
        <button
          onClick={onDismiss}
          className="w-full py-4 bg-secondary hover:bg-hover border border-color rounded-2xl font-bold text-sm text-primary transition-all active:scale-[0.98]"
        >
          Dismiss & Abort
        </button>
