...
import type { UnifiedTransaction, TransactionType } from '../types/allTransaction.types';
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 shrink-0">
                    <div>
                        ...
                        <p className="text-muted text-sm mt-1">
                            History of your account activity
                        </p>
                    </div>
                                ...
                                filteredTransactions.map(tx => (
                                    <tr key={tx.id} className="hover:bg-white/5 transition-colors group">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div
                                                    className={`w-9 h-9 rounded-full flex items-center justify-center border shrink-0 ${tx.type === 'SEND' ? 'bg-warning/10 border-warning/20' :
                                                        tx.type === 'RECEIVE' ? 'bg-success/10 border-success/20' :
                                                            tx.type === 'TRUST' ? 'bg-purple-400/10 border-purple-400/20' :
                                                                tx.type === 'CLAIMABLE' ? 'bg-pink-400/10 border-pink-400/20' :
                                                                    tx.type === 'BRIDGE' ? 'bg-info/10 border-info/20' :
                                        ...
                                        <td className="px-6 py-4">
                                            <span
                                                className={`text-sm font-medium ${getAmountColorClass(tx.type)}`}
                                            >
                                                {getAmountString(tx)}
                                            </span>
                                        </td>
                            ...
                            {hasMore && (
                                <tr>
                                    <td colSpan={5} className="p-0">
                                        <div className="p-4 text-center bg-white/5 hover:bg-white/10 transition-colors cursor-pointer border-t border-white/5" onClick={!isLoading ? loadMore : undefined}>
                                            <button
                                                disabled={isLoading}
