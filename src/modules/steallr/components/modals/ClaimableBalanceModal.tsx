    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-secondary rounded-2xl border border-white/10 w-full max-w-md shadow-2xl overflow-hidden transform transition-all scale-100">
        ...
        <div className="p-2 max-h-[60vh] overflow-y-auto">
          {balances.length === 0 ? (
            <div className="p-4 text-center text-muted text-sm">No claimable balances found.</div>
          ) : (
            <div className="space-y-2 p-2">
              {balances.map(balance => {
                ...
                return (
                  <div
                    key={balance.id}
                    className="bg-white/5 rounded-xl p-4 border border-white/5 flex items-center justify-between group hover:border-pink-500/30 transition-all"
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-muted truncate max-w-[150px]">
                        From: {balance.sponsor?.substring(0, 8)}...
                      </span>
                    </div>
