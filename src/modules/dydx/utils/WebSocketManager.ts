...
class WebSocketManager {
  ...
  private messageQueue: WebSocketSubscription[] = [];
  // Track queued messages as a Set for O(1) duplicate check
  private messageQueueKeys = new Set<string>();
  private flushTimer: ReturnType<typeof setTimeout> | null = null;
  private readonly FLUSH_INTERVAL = 100;
    ...
    return () => {
      ...
      if (handlers) {
        ...
        if (handlers.size === 0) {
          ...
          this.queueMessage({ type: 'unsubscribe', channel, ...(id && { id }) });
        }
      }
    };
  }
  ...
  private resubscribeAll(): void {
    // No need to clear subscriptionInProgress first — onopen already cleared it
    this.pendingSubscriptions.forEach((subscription, key) => {
      this.subscriptionInProgress.add(key);
      this.queueMessage(subscription);
    });
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      if (data.type === 'error' && data.message) {
        ...
        const stats = this.subscriptionStats.get(key);
        if (stats) stats.errorCount++;
        if (isAlreadySubscribed) this.serverSubscriptions.add(key);
        this.subscriptionInProgress.delete(key);
        return;
      }
      ...
      if (data.type === 'channel_data' || data.type === 'channel_batch_data') {
        ...
        // Trades are always unique (sequential), skip dedup entirely
        if (data.channel !== 'v4_trades' && this.isDuplicateMessage(subscriptionKey, data)) {
          return;
        }
  ...
  private throttleMessage(
    ...
    if (throttleInterval === 0) {
      const queue = isHighPriority ? this.pendingHighPriorityCalls : this.pendingHandlerCalls;
      handlers.forEach(handler => queue.push({ handler, data }));
      this.scheduleHandlerExecution(isHighPriority);
      return;
    }
  ...
  private scheduleHandlerExecution(highPriority = false): void {
    if (highPriority) {
      // Guard: only schedule one microtask at a time
  ...
  private startHealthCheck(): void {
    ...
    this.healthCheckInterval = setInterval(() => {
      ...
      this.subscriptionStats.forEach((stats, key) => {
        if (!this.serverSubscriptions.has(key) || stats.lastMessageTime === 0) return;

        const channel = key.split('_').slice(0, 2).join('_');
        const threshold = this.CHANNEL_STALE_THRESHOLDS[channel];
        if (!threshold) return;

        if (now - stats.lastMessageTime > threshold) {
          this.serverSubscriptions.delete(key);
          this.subscriptionInProgress.delete(key);
          const pending = this.pendingSubscriptions.get(key);
          if (pending) {
  ...
  getDebugInfo(): Record<string, unknown> {
    ...
    return {
      ...
      connectionId: this.connectionId,
      currentWsUrl: this.currentWsUrl,
      localSubscriptions: [...this.subscriptions.keys()],
      serverSubscriptions: [...this.serverSubscriptions],
      pendingSubscriptions: [...this.pendingSubscriptions.keys()],
      inProgressSubscriptions: [...this.subscriptionInProgress],
      reconnectAttempts: this.reconnectAttempts,
      isReconnecting: this.isReconnecting,
      reconnectBannerPending: this.disconnectNotifyTimer !== null,
      messageQueueLength: this.messageQueue.length,
      pendingHandlerCalls: this.pendingHandlerCalls.length,
      activeHandlerCount: [...this.subscriptions.values()].reduce((sum, s) => sum + s.size, 0),
      cacheSize: this.messageCache.size,
      activeThrottles: this.throttleMap.size,
      totalMessagesReceived: this.totalMessagesReceived,
      messagesByChannel: Object.fromEntries(this.messagesByChannel),
      subscriptionActivity: [...this.subscriptionStats.entries()].map(([key, stats]) => ({
        key,
        isActive: this.serverSubscriptions.has(key),
        lastMessageAgo: stats.lastMessageTime ? now - stats.lastMessageTime : null,
        totalMessages: stats.messageCount,
