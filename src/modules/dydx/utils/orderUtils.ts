...
export const getDisplayOrderType = (order: OrderLike): string => {
  if (!order) return '—';
  if ((order.clientMetadata === '1' || order.clientMetadata === 1) && order.type === 'LIMIT') {
    return 'MARKET';
  }
  return order.type ?? '—';
};
...
export const computeClosedPnl = (fill: FillLike): PnlResult | null => {
  ...
  // @ts-expect-error - fee may not be typed on FillLike
  ...
  if (fill.positionSideBefore === 'LONG' && fill.side === 'SELL') {
    ...
    const feePortion = fillSize > 0 ? fee * (sizeClosed / fillSize) : 0;
    closedPnl = (fillPrice - entryPrice) * sizeClosed - feePortion;
  } else if (fill.positionSideBefore === 'SHORT' && fill.side === 'BUY') {
    const sizeClosed = Math.min(sizeBefore, fillSize);
    const feePortion = fillSize > 0 ? fee * (sizeClosed / fillSize) : 0;
    closedPnl = (entryPrice - fillPrice) * sizeClosed - feePortion;
  }
  ...
  return {
    ...
    className: isNegative ? 'text-red-400' : closedPnl > 0 ? 'text-green-400' : 'text-primary',
  };
};
...
export const formatTimeAgoCompact = (input: string | number | Date): string => {
  if (!input) return '';
  const date = input instanceof Date ? input : new Date(input);
  if (Number.isNaN(date.getTime())) return '';
