...
const OpenOrdersPanel: React.FC = () => {
  ...
  const getMarketIcon = useCallback((market: string) => {
  ...
  if (ordersError && openOrders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center">
        ...
        <button onClick={refreshOrders} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded transition-colors flex items-center gap-2">
    ...
    <div className="h-full flex flex-col bg-primary overflow-auto">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {sortedOpenOrders.map(order => {
              ...
              return (
                <tr key={order.id} className={`border-b border-color hover:bg-hover transition-colors ${isCancelling ? 'opacity-50' : ''} ${isPending ? 'bg-yellow-500/5' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded text-xs font-bold ${order.side === 'BUY' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {order.side}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center text-muted text-xs">{order.timeInForce || 'GTT'}</td>
                  <td className="px-4 py-3 text-right text-muted text-xs">
                    {order.goodTilBlockTime ? getTimeAgo(order.goodTilBlockTime) : order.updatedAt ? getTimeAgo(order.updatedAt) : '—'}
                  </td>
                  <td className="px-4 py-3 text-center">
                    {!isMarket && (
                      <button onClick={() => handleCancel(order)} disabled={isCancelling}
                        className={`p-1.5 rounded transition-colors ${isCancelling ? 'bg-secondary cursor-not-allowed' : 'bg-red-600 hover:bg-red-500 text-white'}`}>
      ...
      <div className="md:hidden space-y-1.5 p-2">
        {sortedOpenOrders.map(order => {
          ...
          return (
            <div key={order.id} className={`bg-secondary border border-color rounded-lg p-2.5 text-xs ${isCancelling ? 'opacity-50' : ''}`}>
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${order.side === 'BUY' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                    {order.side}
                  </span>
                  {!isMarket && (
                    <button onClick={() => handleCancel(order)} disabled={isCancelling}
                      className={`p-1.5 rounded transition-colors ${isCancelling ? 'bg-primary cursor-not-allowed' : 'bg-red-600 hover:bg-red-500 text-white'}`}>
              ...
              <div className="border-t border-dashed border-color pt-2 grid grid-cols-2 gap-x-3 gap-y-1.5">
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <div>
                    ...
                    {fillPercent > 0 && <div className="text-[9px] text-muted">{fillPercent.toFixed(0)}%</div>}
                  </div>
                </div>
                <div className="flex flex-col gap-0.5">
      ...
      <ConfirmationModal
        ...
        message={
          orderToCancel ? (
            <span>Are you sure you want to cancel your <strong>{orderToCancel.side}</strong> order for <strong>{orderToCancel.ticker}</strong>?</span>
          ) : ''
        }
