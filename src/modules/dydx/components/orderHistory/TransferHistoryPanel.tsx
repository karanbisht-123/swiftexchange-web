...
const TransferHistoryPanel: React.FC = () => {
    ...
    const paginatedTransfers = transfers.slice((localPage - 1) * ITEMS_PER_PAGE, localPage * ITEMS_PER_PAGE);
                ...
                <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-secondary border-b border-color z-10">
                        <tr className="text-muted text-xs">
                            <th className="text-left px-4 py-3 font-medium">Date | <span className="text-primary font-bold">Age</span></th>
                        ...
                        {paginatedTransfers.map((transfer, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`font-medium ${isDeposit ? 'text-green-500' : 'text-primary'}`}>
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-2 group">
                                                <span className="font-mono text-xs text-primary">{truncateAddress(transfer.sender.address)}</span>
                                                <button
                                                ...
                                                >
                                                    {copiedId === `sender-${uniqueKey}` ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                                                </button>
                                            </div>
                                            <div className="flex items-center gap-2 group">
                                                <span className="font-mono text-xs text-muted">{truncateAddress(transfer.recipient.address)}</span>
                                                <button
                                                    onClick={() => handleCopy(transfer.recipient.address, `recipient-${uniqueKey}`)}
                                                    className="opacity-0 group-hover:opacity-100 transition-opacity text-muted hover:text-primary"
                                                >
                                                    {copiedId === `recipient-${uniqueKey}` ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                                                </button>
                                            </div>
                                        </div>
                                    </td>
                                        ...
                                        {transfer.transactionHash && (
                                            <div className="flex items-center justify-end gap-2 group">
                                                <span className="font-mono text-xs text-muted">
                                                    {transfer.transactionHash.slice(0, 4)}...{transfer.transactionHash.slice(-4)}
                                                </span>
                                                <a
                        ...
                        >
                            <div className="flex flex-col gap-1 min-w-0 flex-1">
                                <div className="flex items-center gap-2">
                                    <span className={`font-medium text-sm ${isDeposit ? 'text-green-500' : 'text-primary'}`}>
                                        {isDeposit ? 'Deposit' : 'Withdrawal'}
                                    </span>
