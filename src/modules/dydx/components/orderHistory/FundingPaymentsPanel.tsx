...
const FundingPaymentsPanel: React.FC = () => {
    ...
    if (!loading && payments.length === 0) {
        return <EmptyState title="No Funding Payments" description="No funding payment history found" />;
    }
                        ...
                        {payments.map((payment, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td className={`px-4 py-3 text-right font-mono ${paymentVal >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                                        {paymentVal >= 0 ? '+' : ''}{paymentVal.toFixed(6)}
                                    </td>
                                    <td className={`px-4 py-3 text-right font-mono ${rateVal >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            ...
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex flex-col gap-1">
                                    <div className="flex items-center gap-2">
                                        <SideBadge side={displaySide} />
                                        <span className={`font-mono text-xs ${paymentVal >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                                            {paymentVal >= 0 ? '+' : ''}{paymentVal.toFixed(6)}
                                        </span>
                                    </div>
