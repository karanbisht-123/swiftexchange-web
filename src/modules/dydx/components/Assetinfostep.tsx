...
import { buildSwapUrl, type AssetInfoContext } from '../utils/Depositassetutils';

const AssetIcon: React.FC<{ asset: Asset }> = ({ asset }) => {
    const chainIconUrl = asset.chainId
        ? (() => {
            try {
                const { getChainLogoUrl } = require('../../evm/utils/Chainregistry');
        ...
        <div className="relative shrink-0">
            ...
            {chainIconUrl && (
                <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-background flex items-center justify-center overflow-hidden">
                    <img src={chainIconUrl} alt="" className="w-full h-full object-cover rounded-full border-2 border-background" />
                </div>
            )}
        </div>
    );
            ...
            <div className="overflow-y-auto flex-1 px-6 pb-6 pt-2 flex flex-col gap-6">
                <div className="flex items-center gap-4 px-1 py-2">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="text-sm text-muted font-medium uppercase tracking-wider">{asset.chainName || asset.name}</div>
                    </div>
                </div>
                {isStellar && (
                    <>
                        ...
                        <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                            ...
                            <p className="text-[14px] text-amber-500/90 leading-relaxed font-medium">
                                Stellar USDC requires a quick bridge to an EVM network before depositing. We'll guide you through it.
                            </p>
                        </div>
                    ...
                    <>
                        ...
                        <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                            ...
                            <div className="text-[14px] text-amber-500/90 leading-relaxed font-medium">
                                <span className="font-bold">USDC Required.</span> dYdX exclusively accepts USDC. Let's swap your <span className="font-bold">{asset.symbol}</span> first.
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-5 rounded-xl border border-color bg-tertiary/20">
                            ...
                            {/* To */}
                            <div className="flex flex-col items-center gap-2">
                                <img src="https://assets.coingecko.com/coins/images/6319/small/usdc.png" alt="USDC" className="w-10 h-10 rounded-full" />
