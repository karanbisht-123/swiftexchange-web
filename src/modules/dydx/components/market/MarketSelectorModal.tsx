...
const MarketRow = memo(function MarketRow({
    ...
    return (
        <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-[#1e293b]/60 transition-colors border-b border-[#1e293b]/30 ${isSelected ? 'bg-blue-500/10 border-l-2 border-l-blue-500' : ''
                }`}
        >
            {/* Favorite button */}
            <button
            ...
            <div className="text-right flex-shrink-0">
                ...
                <div
                    className={`text-xs font-medium flex items-center justify-end gap-0.5 ${isPositive ? 'text-emerald-400' : 'text-red-400'
                        }`}
                >
                    {isPositive ? (
...
export default function MarketSelectorModal({ isOpen, onClose }: MarketSelectorModalProps) {
    ...
    const handleSelect = useCallback((ticker: string, data: MarketData) => {
    ...
    const filteredMarkets = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(
                m => m.ticker.toLowerCase().includes(term) ||
                    m.coinName?.toLowerCase().includes(term)
            );
        }
    ...
    return (
        <>
            ...
            <div
                ref={panelRef}
                className={`fixed top-0 left-0 z-50 h-full w-full max-w-sm bg-secondary border-r border-[#334155] shadow-2xl transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                {/* Header */}
                ...
                <div className="flex border-b border-[#334155]/50">
                    <button
                        ...
                        className={`flex-1 px-4 py-2.5 text-sm font-medium transition-colors ${activeTab === 'all'
                    ...
                    <button
                        ...
                        className={`flex-1 px-4 py-2.5 text-sm font-medium transition-colors flex items-center justify-center gap-1.5 ${activeTab === 'favorites'
                            ? 'text-white border-b-2 border-blue-500'
                            : 'text-slate-400 hover:text-slate-300'
                            }`}
                    >
