        ...
        <div className="flex items-center gap-1 overflow-x-auto hide-scrollbar px-1 flex-1">
            {TIMEFRAMES.map(tf => (
                <button
                    ...
                    className={`px-2 py-1 text-[11px] font-medium rounded transition-all whitespace-nowrap ${value === tf.value ? 'bg-brand text-white' : 'text-muted hover:text-primary hover:bg-hover'
                        }`}
                >
                    {tf.label}
                </button>
                    ...
                    <div className="absolute top-full left-0 mt-1 bg-secondary rounded-lg shadow-xl border border-color py-1 min-w-[140px] z-20">
                        {CHART_TYPES.map(ct => (
                            <button
                                ...
                                className={`w-full text-left px-3 py-2.5 text-xs hover:bg-hover transition-colors flex items-center gap-2 ${value === ct.value ? 'bg-hover text-brand font-medium' : 'text-primary'
                                    }`}
                            >
                                {ct.icon}
                                <span>{ct.label}</span>
                            ...
                            {filteredIndicators.map(ind => {
                                ...
                                return (
                                    <button
                                        key={ind.id}
                                        onClick={() =>
                                            onToggleIndicator(ind.id, firstActive?.instanceId ?? null)
                                        }
    ...
    const renderToggleRow = (
        ...
        <div className="w-full px-4 py-2.5 hover:bg-hover transition-colors flex items-center justify-between group">
            ...
            >
                ...
                <div
                    className={`w-9 h-5 rounded-full transition-colors ${value ? 'bg-brand' : 'bg-gray-600'
                        } relative shrink-0`}
                >
                    <div
