...
export const DrawingToolbar = memo(function DrawingToolbar({
    ...
    const activeCatObj: ToolCategory | undefined = TOOL_CATEGORIES.find(
    ...
    return (
      ...
      `}</style>
            ...
            <button
                ...
                className={`p-2 rounded-lg transition-colors flex items-center justify-center min-w-[32px] min-h-[32px] ${!activeTool ? 'bg-brand/15 text-brand' : 'text-gray-400 hover:bg-hover hover:text-primary'
                    }`}
                title="Cursor"
            >
            ...
            {TOOL_CATEGORIES.map(cat => {
                const isCatActive = cat.sections.some(sec =>
                    sec.tools.some(t => t.id === activeTool)
                );
                ...
                return (
                    <div key={cat.id} className="relative">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-colors flex items-center justify-center min-w-[32px] min-h-[32px] relative ${isActive
                                        ...
                                        {sec.tools.map(tool => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-md text-left text-xs transition-colors group ${isToolActive
                                                            ? 'bg-brand/15 text-brand font-medium border border-brand/20'
                                                            : 'text-gray-400 hover:bg-hover hover:text-primary'
                                                        }`}
                                                >
                                                    <span
                                                        className={`shrink-0 ${isToolActive
                                                                ? 'text-brand'
...
// ============================================================================
// DrawingStyleBar — color + width picker shown above chart while placing
// ============================================================================
            ...
            <div className="flex items-center gap-1.5 border-r border-color pr-3">
                {DRAWING_COLORS.map(c => (
                    <button
                        ...
                        className={`w-4 h-4 rounded-full border transition-all ${color === c.hex ? 'scale-125 border-white' : 'border-transparent'
                            }`}
            ...
            <div className="flex items-center gap-1">
                {DRAWING_WIDTHS.map(w => (
                    <button
                        ...
                        className={`px-2 py-0.5 text-[10px] font-bold rounded transition-colors ${width === w ? 'bg-brand text-white' : 'text-muted hover:bg-hover hover:text-primary'
                            }`}
                    >
                        {w}px
                    </button>
...
export const ScaleModeToggle = memo(function ScaleModeToggle({
    ...
    return (
        <div className="absolute bottom-6 right-[68px] z-20 flex items-center gap-1 bg-secondary border border-color rounded p-0.5 shadow-md pointer-events-auto select-none">
            <button
                onClick={onToggle}
                className={`px-1.5 py-0.5 text-[9px] font-bold uppercase rounded transition-colors ${isLog ? 'bg-brand text-white' : 'text-muted hover:text-primary'
                    }`}
