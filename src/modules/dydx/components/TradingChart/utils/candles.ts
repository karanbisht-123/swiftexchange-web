...
export function normalizeCandles(raw: any[]): CandleBar[] {
    return raw
        .map(c => ({
            time: Math.floor(new Date(c.startedAt).getTime() / 1000),
...
export function getPlot(
    ...
    for (const key of keys) {
        if (Array.isArray(plots?.[key]) && plots[key].length > 0) {
            return plots[key].filter(
                (d: any) => d && typeof d.value === 'number' && isFinite(d.value)
            );
        }
    }
    return [];
...
export function candlesMatchMarket(candles: any[], selectedMarket: string): boolean {
    ...
    return (
        ...
        selectedMarket.startsWith(ticker)
    );
}