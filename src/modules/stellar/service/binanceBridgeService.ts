      ...
      if (this.ws === ws) {
        ...
        if (!hasOpened) {
          ...
          if (connectionAttempts >= MAX_ATTEMPTS) {
            console.warn(
              '[BinanceSocketManager] Failed to connect to Binance WebSocket. Falling back to Stellar Horizon.'
            );
...
export class BinanceBridgeService {
  private static handleFetchFailure(err: any) {
    if (!isBinanceFailed()) {
      console.warn(
        '[BinanceBridgeService] REST API call failed. Falling back to Stellar Horizon.',
        err
      );
      binanceFailedAt = Date.now();
      window.dispatchEvent(new CustomEvent('binance:connection-failed'));
