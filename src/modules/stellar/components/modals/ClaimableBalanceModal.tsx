import { AlertCircle, CheckCircle, Gift, Loader2, PartyPopper, ShoppingBag, X } from 'lucide-react';
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-6 animate-in fade-in zoom-in duration-500">
              ...
              {lastHash && (
                <div className="w-full bg-white/5 rounded-2xl p-4 border border-white/5 space-y-2">
                  <p className="text-[10px] font-bold text-muted uppercase tracking-widest text-left">
                    Transaction Hash
                  </p>
                  <div className="flex items-center justify-between gap-2">
                    <code className="text-[10px] font-mono text-green-400 truncate flex-1">
                      {lastHash}
                    </code>
                    <button
                      onClick={() =>
                        window.open(
                          `https://stellar.expert/explorer/public/tx/${lastHash}`,
                          '_blank'
                        )
                      }
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 animate-in fade-in zoom-in duration-500">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted max-w-[240px] mx-auto">
                  You don't have any pending claimable balances at the moment.
                </p>
              </div>
              <button
                onClick={onClose}
                className="btn bg-white/5 hover:bg-white/10 text-primary px-8 border border-white/10"
              >
                Back to Trade
              </button>
            </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] font-bold text-muted uppercase tracking-tighter">
                          Sponsored by {balance.sponsor?.substring(0, 4)}...
                          {balance.sponsor?.substring(balance.sponsor.length - 4)}
                        </span>
                      </div>
                    </div>
