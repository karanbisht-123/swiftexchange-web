import { ChevronDown, RefreshCw, TrendingUp, Wallet, X } from 'lucide-react';
...
const CryptoMarket = () => {
  ...
  const renderContent = () => {
    if (!evmWallet) {
      return (
        <div className="flex items-center justify-center h-64 text-primary mt-12">
          <div className="text-center px-5 py-8 max-w-sm mx-auto">
            ...
            <p className="font-medium text-primary mb-1">Connect Wallet to View Market</p>
            <p className="text-sm text-muted mb-6">
              Connect your EVM wallet to view live market data.
            </p>
            <button
              onClick={openModal}
              className="btn btn-primary px-6 py-2.5 rounded-xl font-bold text-sm inline-flex items-center gap-2 mx-auto"
            >
              <Wallet size={16} />
              Connect Wallet
            </button>
          ...
          {data.map(coin => (
            ...
            >
              ...
              <div className="text-right shrink-0">
                ...
                <div
                  className={`text-xs font-medium mt-0.5 ${coin.priceChangePercentage24h >= 0 ? 'text-success' : 'text-danger'}`}
                >
        ...
        <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
          ...
          <thead>
            <tr className="bg-primary">
              ...
              <th className="px-4 py-3 text-right text-xs font-medium text-muted uppercase tracking-wider hidden md:table-cell">
                High
              </th>
              <th className="px-4 py-3 text-right text-xs font-medium text-muted uppercase tracking-wider hidden md:table-cell">
                Low
              </th>
              <th className="px-4 py-3 text-right text-xs font-medium text-muted uppercase tracking-wider hidden lg:table-cell">
                Mkt cap
              </th>
              <th className="px-4 py-3 text-right text-xs font-medium text-muted uppercase tracking-wider hidden lg:table-cell">
                Volume
            ...
            {data?.map(coin => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right">
                  <span
                    className={`text-xs font-medium ${coin.priceChangePercentage24h >= 0 ? 'text-success' : 'text-danger'}`}
                  >
                    {coin.priceChangePercentage24h >= 0 ? '+' : ''}
                    {coin?.priceChangePercentage24h?.toFixed(2)}%
                  </span>
          ...
          >
            ...
            <div className="shrink-0 flex items-center gap-3 px-5 py-4 border-b border-color/40">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-primary text-base leading-tight truncate">
                  {c.name}
                </div>
              ...
              <div className="px-5 py-5 text-center border-b border-color/30">
                ...
                <div className="flex items-center justify-center gap-2 mt-3">
                  ...
                  >
                    <TrendingUp
                      size={12}
                      className={c.priceChangePercentage24h < 0 ? 'rotate-180' : ''}
                    />
              ...
              <div className="px-2 py-4 border-b border-color/30">
                ...
                <div className="space-y-0">
                  <div className="flex items-center justify-between py-2.5 border-b border-color/20">
                    ...
                    <div className="text-right">
                      ...
                      {c.marketCapChangePercentage_24h !== undefined && (
                        <p
                          className={`text-xs mt-0.5 ${c.marketCapChangePercentage_24h >= 0 ? 'text-success' : 'text-danger'}`}
                        >
                          {c.marketCapChangePercentage_24h >= 0 ? '+' : ''}
                          {c.marketCapChangePercentage_24h?.toFixed(2)}%
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between py-2.5 border-b border-color/20">
                    ...
                    <span className="text-sm font-medium text-primary">
                      {formatLargeNumber(c.totalVolume)}
                    </span>
                  </div>
                  ...
                  {c.fullyDilutedValuation && (
                    <div className="flex items-center justify-between py-2.5">
                      ...
                      <span className="text-sm font-medium text-primary">
                        {formatLargeNumber(c.fullyDilutedValuation)}
                      </span>
                    </div>
                  )}
                  ...
                  {c.roi && (
                    <div className="flex items-center justify-between py-2.5 border-t border-color/20">
                      ...
                      <span
                        className={`text-sm font-medium ${c.roi.percentage >= 0 ? 'text-success' : 'text-danger'}`}
                      >
                ...
                {c.atl && (
                  <div className="px-5 py-4">
                    ...
                    <p className="text-xs text-success mt-1">
                      +{c.atlChangePercentage?.toFixed(2)}%
                    </p>
