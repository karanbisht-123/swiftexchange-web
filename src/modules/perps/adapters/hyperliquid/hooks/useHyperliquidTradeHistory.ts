...
import { getHyperliquidUserFills } from '../api/history';
...
export const useHyperliquidTradeHistory = (userAddr: string | null, symbol: string | null) => {
  const cacheKey = userAddr ? `${userAddr}_${symbol || 'all'}_trades_hl` : '';
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const data = await getHyperliquidUserFills(userAddr, {
          limit: PAGE_LIMIT,
        });

        let filtered = data;
        if (symbol) {
          filtered = data.filter(t => t.symbol === symbol);
        }
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const nextBatch = await getHyperliquidUserFills(userAddr, {
      ...
      if (list.length === 0) {
        ...
        setTrades(prev => {
          ...
          const more = nextBatch.length >= PAGE_LIMIT; // Determine if hasMore based on API batch size
          setHasMore(more);
  ...
  }, [userAddr, symbol, isLoadingMore, hasMore, trades, cacheKey]);
