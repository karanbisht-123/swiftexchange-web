...
const DepthChart: React.FC<DepthChartProps> = ({ activeChartTab, onChartTabChange }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const maxSpread = Math.max(maxBidSpread, maxAskSpread, 0.0001);
  ...
  return (
    <div className="w-full h-full bg-secondary text-primary flex flex-col">
      ...
      {depthData && (
        <div className="flex items-center justify-center gap-6 px-4 py-3 border-t border-color text-xs shrink-0 bg-secondary">
