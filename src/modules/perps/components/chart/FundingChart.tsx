...
const FundingChart: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const historicalData = await getFundingRates(asterSymbol, 500);
        // Aster API returns an array of { fundingRate, fundingTime }
        const sorted = historicalData.sort((a, b) => a.fundingTime - b.fundingTime);
        setData(sorted);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (toolTipRef.current) {
      ...
      chart.subscribeCrosshairMove(param => {
        ...
        const date = new Date(item.fundingTime).toLocaleString();
  ...
  if (loading && data.length === 0) {
    return (
      <div className="flex flex-col h-full w-full bg-secondary text-primary relative group justify-center items-center">
        <span className="text-muted text-sm">Loading funding chart...</span>
      </div>
    );
  }
