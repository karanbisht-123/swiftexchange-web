import { Download, RefreshCw, Sliders, X as XIcon } from 'lucide-react';
      ...
      <div className="relative w-full max-w-4xl bg-secondary border border-(--color-border) rounded-3xl shadow-premium p-6 md:p-8 animate-modal-scale-in max-h-[90vh] flex flex-col z-10 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-(--color-text-secondary) mt-0.5">
                Specify opening amounts and cost per unit for your Stellar assets held prior to this
                period.
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-6 max-h-[60vh] custom-scrollbar">
          <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 text-amber-400 text-xs flex gap-2">
            <span className="shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-amber-500/10 font-bold select-none text-[11px]">
              !
            </span>
            <div>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-(--color-border)/40">
                    {stellarPnlData.positions.map((pos: any, idx: number) => {
                      ...
                      const amtValText =
                        config.openingAmount !== ''
                          ? config.openingAmount
                          : autoVal?.amount?.toString() || '';
                      const cpuValText =
                        config.costPerUnit !== ''
                          ? config.costPerUnit
                          : autoVal?.price?.toString() || '';
                      ...
                      const unrealPnl =
                        amtValText !== '' && cpuValText !== ''
                          ? pos.remaining * currentPrice - openCost
                          : (pos.unrealized ?? 0);

                      const isAutoAmount =
                        config.openingAmount === '' && autoVal?.amount !== undefined;
                      const isAutoPrice = config.costPerUnit === '' && autoVal?.price !== undefined;

                      return (
                        <tr
                          key={pos.asset}
                          className={`${idx % 2 === 0 ? '' : 'bg-(--color-bg-tertiary)/10'} hover:bg-(--color-bg-tertiary)/20 transition-colors`}
                        >
                          <td className="px-5 py-4 font-bold text-(--color-text-primary)">
                            {pos.asset}
                          </td>
                          ...
                          <td className="px-5 py-3 text-center">
                            <div className="relative inline-block w-full max-w-[120px]">
                              <input
                                ...
                                value={amtValText}
                                onChange={e =>
                                  handleCostBasisChange(pos.asset, 'openingAmount', e.target.value)
                                }
                                placeholder="0.00"
                                ...
                                title={
                                  isAutoAmount
                                    ? 'Auto-filled from backend history. Edit to customize.'
                                    : 'Custom value'
                                }
                              />
                            </div>
                          ...
                          <td className="px-5 py-3 text-center">
                            <div className="relative inline-block w-full max-w-[120px]">
                              <input
                                ...
                                value={cpuValText}
                                onChange={e =>
                                  handleCostBasisChange(pos.asset, 'costPerUnit', e.target.value)
                                }
                                placeholder="0.00"
                                className={`w-full text-center font-semibold font-mono text-xs px-2.5 py-1.5 rounded-lg border focus:ring-2 focus:ring-brand-primary/10 outline-none transition-all duration-200 ${
                                  isAutoPrice
                          ...
                          <td
                            className={`px-5 py-4 text-right font-black ${unrealPnl >= 0 ? 'text-green-400' : 'text-red-400'}`}
                          >
                            {unrealPnl >= 0 ? '+' : ''}
                            {portfolioUtils.formatUSD(unrealPnl)}
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div className="grid grid-cols-1 gap-4 md:hidden">
                {stellarPnlData.positions.map((pos: any) => {
                  ...
                  return (
                    <div
                      key={pos.asset}
                      className="p-4 bg-gradient-to-br from-(--color-bg-secondary) to-(--color-bg-tertiary)/30 border border-(--color-border) rounded-2xl space-y-4 shadow-sm relative overflow-hidden group"
                    >
                      ...
                      <div className="flex items-center justify-between border-b border-(--color-border)/40 pb-2">
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-sm text-(--color-text-primary)">
                            {pos.asset}
                          </span>
                          {(isAutoAmount || isAutoPrice) && (
                            <span className="text-[8px] font-bold uppercase bg-purple-500/10 text-purple-400 border border-purple-500/20 px-1 py-0.2 rounded-full">
                              Auto
                            </span>
                          )}
                        </div>
                        <span className="text-[10px] uppercase font-bold text-(--color-text-secondary)">
                          Position Data
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="space-y-0.5">
                          <span className="text-[9.5px] uppercase font-bold text-(--color-text-secondary) tracking-wider block">
                            Wallet Balance
                          </span>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        <div className="space-y-1">
                          <label className="text-[9px] uppercase font-bold text-(--color-text-secondary) tracking-wider block">
                            Opening Amount
                          </label>
                          <input
                            type="text"
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-3 border-t border-(--color-border)/30 text-xs">
                        <div className="space-y-0.5">
                          <span className="text-[9px] uppercase font-bold text-(--color-text-secondary) tracking-wider block">
                            Opening Cost
                          </span>
                        ...
                        <div className="space-y-0.5 text-right">
                          ...
                          <span
                            className={`font-black ${unrealPnl >= 0 ? 'text-green-400' : 'text-red-400'}`}
                          >
                            {unrealPnl >= 0 ? '+' : ''}
                            {portfolioUtils.formatUSD(unrealPnl)}
                          </span>
