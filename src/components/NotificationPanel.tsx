...
const typeConfig: Record<NotificationType, { icon: React.ReactNode; color: string }> = {
    EVM_SWAP: {
        icon: <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-500/10 text-sm text-blue-500">🔄</div>,
        color: 'text-blue-500'
    },
    SEND: {
        icon: <div className="flex h-8 w-8 items-center justify-center rounded-full bg-green-500/10 text-sm text-green-500">↗️</div>,
        color: 'text-green-500'
    },
    RECEIVE: {
        icon: <div className="flex h-8 w-8 items-center justify-center rounded-full bg-purple-500/10 text-sm text-purple-500">↙️</div>,
        color: 'text-purple-500'
    },
    BRIDGE: {
        icon: <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-500/10 text-sm text-indigo-500">🌉</div>,
        color: 'text-indigo-500'
    },
    STELLAR: {
        icon: <div className="flex h-8 w-8 items-center justify-center rounded-full bg-yellow-500/10 text-sm text-yellow-500">⭐</div>,
    ...
    SYSTEM: {
        icon: <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-500/10 text-sm text-gray-400">⚙️</div>,
        color: 'text-gray-500'
    },
};
...
export const NotificationPanel: React.FC<NotificationPanelProps> = ({
    ...
    const displayNotifications = notifications.filter(n => {
        ...
        if (search && !n.title.toLowerCase().includes(search.toLowerCase()) &&
            typeof n.message === 'string' && !n.message.toLowerCase().includes(search.toLowerCase())) {
            return false;
        }
        return true;
    });
                        ...
                        {displayNotifications.map((notif) => (
                            ...
                            >
                                ...
                                <div className="min-w-0 flex-1 relative group/item">
                                    ...
                                    <div className="break-words text-[13px] text-gray-400 pr-6">
                                        {notif.message}
                                    </div>
                                    <button
                                        onClick={(e) => {
