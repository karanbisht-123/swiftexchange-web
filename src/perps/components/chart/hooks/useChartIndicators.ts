...
export interface UseChartIndicatorsResult {
  ...
  applyIndicatorsTick: (activeIndicators: ActiveIndicator[]) => void;
  scheduleIndicatorRecalc: (updatedBar: CandleBar) => void;
  setRecalcContext: (ctx: { activeIndicators: ActiveIndicator[] }) => void;
}
    ...
    (
      ...
      activeIndicators.forEach(active => {
        ...
        try {
          ...
          logDev(`[Chart] Indicator calculation error (${active.indicatorId}):`, err);
          seriesVal.seriesList.forEach(series => {
            try {
              series.setData([]);
            } catch (clearErr) {
              logDev('[Chart] Failed to clear indicator series:', clearErr);
            }
          });
        }
      });
      ...
      if (seriesRef.current) {
        ...
        try {
          ...
          logDev('[Chart] Error setting candlestick markers:', err);
        }
      }
  ...
  const applyIndicatorsTick = useCallback((activeIndicators: ActiveIndicator[]) => {
    activeIndicators.forEach(active => {
      ...
      try {
        ...
        (entry.plotConfig || []).forEach(plot => {
          ...
          if (lastPoint && typeof lastPoint.value === 'number' && isFinite(lastPoint.value)) {
            try {
              ...
              logDev(`[Chart] Failed to update series point (${active.indicatorId}):`, updateErr);
            }
          }
        });
          ...
          entry.plotCandleConfig.forEach(pc => {
            ...
            if (lastCandle) {
              try {
                ...
                logDev(
                  `[Chart] Failed to update candle series (${active.indicatorId}):`,
                  updateErr
                );
              }
            }
        ...
        logDev(`[Chart] Indicator tick error (${active.indicatorId}):`, err);
      }
    });
        ...
        if (!activeIds.has(instId)) {
          val.seriesList.forEach(series => {
            try {
              ...
              logDev('[Chart] Failed to remove series:', err);
            }
          });
          ...
          if (entry.group === 'candlestick') {
            ...
            if (paneIndex > 0) {
              try {
                ...
                logDev('[Chart] Failed to set pane height:', err);
              }
            }
            ...
            (entry.plotConfig || []).forEach(plot => {
              ...
              if (series && plot) {
                try {
                  ...
                  logDev('[Chart] Failed to update series options:', err);
                }
              }
            });
            if (entry.plotCandleConfig) {
              entry.plotCandleConfig.forEach(() => {
                ...
                if (series) {
                  try {
                    ...
                    logDev('[Chart] Failed to update candle series options:', err);
                  }
                }
              });
            }
