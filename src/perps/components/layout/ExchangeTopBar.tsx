...
export const ExchangeTopBar: React.FC = () => {
  ...
  const changePctDisplay = !isNaN(changePct) && isFinite(changePct) ? `${isChangePositive ? '+' : ''}${changePct.toFixed(2)}%` : '--';

  const isFundingPositive = liveFundingRate >= 0;

  return (
    <>
      <div className="flex items-center px-4 h-[60px] border-b border-color bg-secondary shrink-0 w-full relative overflow-hidden min-w-0 max-w-[740px]">
          ...
          >
            ...
            <div className="flex flex-col items-start gap-1.5">
              <div className="flex items-center gap-1.5 leading-none">
                <span className="text-primary font-bold text-[14px]">{symbol.replace('-', '')}</span>
                <span className="text-[#a0a5ad] text-[9px] bg-tertiary border border-color px-1 py-0.5 rounded font-medium">Perp</span>
              </div>
              <div className="flex items-center gap-2 leading-none">
                <span className={`text-[12px] font-semibold ${isChangePositive ? 'text-success' : 'text-danger'}`}>
                  {formatPrice(markPrice)}
                </span>
                <span className={`text-[10px] font-medium ${isChangePositive ? 'text-success' : 'text-danger'}`}>
                  {changePctDisplay}
                </span>
              </div>
            </div>
            <svg className="text-muted ml-1" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
          </button>
        ...
        <div className="flex items-center gap-8 overflow-x-auto [&::-webkit-scrollbar]:hidden whitespace-nowrap flex-1 min-w-0 h-full px-2 mask-edges">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-muted text-[10px] uppercase tracking-wider font-medium">Mark Price</span>
          ...
          {/* Index Price */}
          <div className="flex flex-col gap-1">
            <span className="text-muted text-[10px] uppercase tracking-wider font-medium">Index Price</span>
          ...
          {/* 24h Change */}
          <div className="flex flex-col gap-1">
            <span className="text-muted text-[10px] uppercase tracking-wider font-medium">24h Change</span>
            <span className={`text-[13px] font-semibold ${isChangePositive ? 'text-success' : 'text-danger'}`}>
              {changePctDisplay}
            </span>
          </div>
          ...
          {/* 24h Volume */}
          <div className="flex flex-col gap-1">
            <span className="text-muted text-[10px] uppercase tracking-wider font-medium">24h Volume (USDT)</span>
            <span className="text-primary text-[13px] font-semibold">{formatLargeNumber(dayNtlVlm)}</span>
          </div>

          {/* Open Interest */}
          <div className="flex flex-col gap-1">
            <span className="text-muted text-[10px] uppercase tracking-wider font-medium">Open Interest</span>
            <span className="text-primary text-[13px] font-semibold">{formatLargeNumber(openInterest)}</span>
          </div>

          {/* Funding */}
          <div className="flex flex-col gap-1 pr-4">
            <span className="text-muted text-[10px] uppercase tracking-wider font-medium">Funding({fundingIntervalHours}h)/Countdown</span>
            <div className="flex items-center gap-1">
              <span className={`text-[13px] font-semibold ${isFundingPositive ? 'text-success' : 'text-danger'}`}>
                {(!isNaN(liveFundingRate) && isFinite(liveFundingRate)) ? (liveFundingRate * 100).toFixed(4) + '%' : '--'}
              </span>
          ...
          {/* Right Fixed Area (Exchange Selector) */}
