  ...
  return (
    <div className="w-[300px] shrink-0 bg-secondary border border-color rounded-md overflow-hidden flex flex-col h-full">
      <div className="flex items-center gap-6 px-4 h-10 border-b border-color text-[11px] text-secondary shrink-0">
        <button className="text-primary font-medium border-b-2 border-brand h-full px-1">Book</button>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-color text-[11px] text-muted shrink-0">
        <div className="flex items-center gap-1.5">
          <button className="w-5 h-5 rounded bg-tertiary flex items-center justify-center hover:bg-hover">-</button>
          <button className="w-5 h-5 rounded bg-tertiary flex items-center justify-center hover:bg-hover">+</button>
      ...
      <div className="flex items-center justify-between px-4 py-1.5 text-[10px] text-muted font-medium shrink-0">
        ...
        <div className="flex items-center gap-4">
          <span>Size <span className="bg-tertiary px-1 py-0.5 rounded ml-0.5">{base}</span></span>
          <span>Total <span className="bg-tertiary px-1 py-0.5 rounded ml-0.5">{quote}</span></span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden font-mono text-[11px] font-medium">
        <div className="flex flex-col flex-1 justify-end pb-1 overflow-hidden">
          {asks.length === 0 ? (
            ...
            asks.map((ask, i) => {
              ...
              return (
                <div key={`ask-${ask.price}-${i}`} className="flex justify-between items-center px-4 py-[3px] hover:bg-hover cursor-pointer relative leading-none shrink-0">
                  <div className="absolute right-0 top-0 bottom-0 bg-danger-bg opacity-[0.15] pointer-events-none" style={{ width: `${(sizeVal / maxSize) * 100}%` }}></div>
                  ...
                  <div className="flex items-center gap-3 z-10">
                    ...
                    <span className="text-secondary w-16 text-right">{formatCurrency(totalVal, true)}</span>
                  </div>
                </div>
              );
            })
        ...
        <div className="flex items-center justify-between px-4 py-1.5 border-y border-color bg-primary shrink-0 text-sm">
          <div className="flex items-center gap-2">
            <span className="text-danger font-bold text-[13px]">{lowestAsk > 0 ? formatCurrency(lowestAsk) : '-'}</span>
          </div>
        ...
        <div className="flex flex-col flex-1 pt-1 overflow-hidden">
          {bids.length === 0 ? (
            ...
            bids.map((bid, i) => {
              ...
              const totalVal = sizeVal * priceVal;
              return (
                <div key={`bid-${bid.price}-${i}`} className="flex justify-between items-center px-4 py-[3px] hover:bg-hover cursor-pointer relative leading-none shrink-0">
                  <div className="absolute right-0 top-0 bottom-0 bg-success-bg opacity-[0.15] pointer-events-none" style={{ width: `${(sizeVal / maxSize) * 100}%` }}></div>
