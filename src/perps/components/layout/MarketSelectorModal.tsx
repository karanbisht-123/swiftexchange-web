...
import { useMarketStore, marketStore } from '../../core/stores/marketStore';
...
export const MarketSelectorModal: React.FC<MarketSelectorModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      >
      <div
        className="w-full sm:max-w-3xl bg-[#0b0e14] border border-color rounded-t-xl sm:rounded-xl flex flex-col shadow-2xl font-body h-[85vh] sm:h-auto max-h-[85vh] animate-slide-up-modal"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Search */}
        <div className="p-4 border-b border-color">
          <div className="relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
            <input
              type="text"
        ...
        <div className="p-3 border-t border-color bg-secondary flex items-center gap-4 text-[10px] text-muted rounded-b-xl shrink-0">
          <div className="flex items-center gap-1.5"><span className="bg-tertiary px-1.5 py-0.5 rounded border border-color font-mono">↵</span> Select</div>
          <div className="flex items-center gap-1.5"><span className="bg-tertiary px-1.5 py-0.5 rounded border border-color font-mono">Esc</span> Close</div>
        </div>
      </div>
    </div>
    </>
...
const MarketRow = React.memo(function MarketRow({
  ...
  const formatPrice = (v: number) =>
    v > 0 ? `$${v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}` : '—';
  ...
  return (
    ...
    >
      ...
      <div className={`text-right text-[13px] font-medium ${isPositive ? 'text-success' : 'text-danger'}`}>
