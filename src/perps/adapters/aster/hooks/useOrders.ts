...
export function useOrders(signer: Signer | null, userAddr: string | null) {
  ...
  const [cancelState, setCancelState] = useState<AsyncState<AsterOrderResponse | { code: number; msg: string }>>(initState());
  const [openOrdersState, setOpenOrdersState] = useState<AsyncState<AsterOrderResponse[]>>(initState());
  const [orderHistoryState, setOrderHistoryState] = useState<AsyncState<AsterOrderResponse[]>>(initState());

  const place = useCallback(async (params: PlaceOrderParams) => {
    ...
    try {
      ...
      setPlaceState({ data: null, loading: false, error: e instanceof Error ? e : new Error(String(e)) });
    }
  }, [signer, userAddr]);

  const placeBatch = useCallback(async (orders: PlaceOrderParams[]) => {
    if (!signer || !userAddr) return;
  ...
  const cancel = useCallback(async (params: CancelOrderParams) => {
    ...
    try {
      ...
      setCancelState({ data: null, loading: false, error: e instanceof Error ? e : new Error(String(e)) });
    }
  }, [signer, userAddr]);

  const cancelAll = useCallback(async (symbol: string) => {
    if (!signer || !userAddr) return;
    return cancelAllOpenOrders(signer, userAddr, symbol);
  }, [signer, userAddr]);

  const cancelBatch = useCallback(async (params: CancelBatchParams) => {
    if (!signer || !userAddr) return;
    return cancelBatchOrders(signer, userAddr, params);
  }, [signer, userAddr]);

  const fetchQuery = useCallback(async (params: Parameters<typeof queryOrder>[2]) => {
    if (!signer || !userAddr) return;
  ...
  const fetchOpenOrders = useCallback(async (symbol?: string) => {
    ...
    try {
      ...
      setOpenOrdersState({ data: null, loading: false, error: e instanceof Error ? e : new Error(String(e)) });
    }
  }, [signer, userAddr]);

  const fetchOrderHistory = useCallback(async (params: GetAllOrdersParams) => {
    if (!signer || !userAddr) return;
    ...
    try {
      ...
      setOrderHistoryState({ data: null, loading: false, error: e instanceof Error ? e : new Error(String(e)) });
    }
  }, [signer, userAddr]);
